...
def rag_chain_pipeline():
    ...
    loader  = PyPDFLoader(file_path="your file path")
    docs = loader.load()

    # Now we will split the data into smaller chunks and store in vector database (Chroma)
    text_splitter  = RecursiveCharacterTextSplitter(chunk_size = 1000,chunk_overlap=200)
    split = text_splitter.split_documents(docs)
    vectore_db = Chroma.from_documents(documents=split,embedding=CohereEmbeddings(model="embed-english-v3.0",cohere_api_key="Your api key"))
